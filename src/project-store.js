...
export function projectOrder(items,query='',trash=false){return items.filter(x=>!x.purged&&!!x.deleted===trash&&x.name.toLowerCase().includes(query.toLowerCase())).sort((a,b)=>Number(!!b.pinned)-Number(!!a.pinned)||b.updatedAt-a.updatedAt);}
...
export const trashProjects=(ids,action)=>transaction('readwrite',(s,done)=>{
 if(!['restore','delete'].includes(action))throw Error('Invalid Trash action.');
 for(const id of new Set(ids)){const r=s.get(id);r.onsuccess=()=>{const entry=r.result;if(!entry?.deleted||entry.purged)return;
  if(action==='restore')s.put({...entry,deleted:false},id);
  else if(id==='legacy-recovery')s.put({id,purged:true,deleted:true},id);
  else s.delete(id);
 };}
 done(true);
});
