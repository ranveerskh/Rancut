...
export default function UpdatePanel(){
 ...
 const upToDate=!!info.checked&&info.releaseState==='published'&&!info.available&&!info.blocked;
 const noRelease=info.releaseState==='none';
 const statusText=message||info.error||(upToDate?`RanCut ${current} is current.`:info.phase==='idle'&&!info.checked?'Click Check for updates to retry.':'');
 return <>
  ...
  {noRelease&&<div className="helpCallout"><b>No installer published yet</b><span>A GitHub code push or Actions build alone does not publish an in-app update. Publish a higher-version GitHub Release with its RanCut Setup.exe and release-metadata.json assets.</span></div>}
