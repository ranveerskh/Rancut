...
test('Home supports named projects, rename, duplicate, trash, restore and persisted reopen',async()=>{
 ...
 click('Trash',doc().querySelector('.homeFilters'));await until(()=>doc().querySelector('.projectCardActions button')?.textContent==='Restore project');click('Restore project');await until(()=>doc().querySelectorAll('.projectCard').length===0);click('Your projects');await until(()=>doc().querySelectorAll('.projectCard').length===2);
 const copy=[...doc().querySelectorAll('.projectCard')].find(c=>c.textContent.includes('— copy'));click('Trash',copy);await until(()=>doc().querySelectorAll('.projectCard').length===1);click('Trash',doc().querySelector('.homeFilters'));await until(()=>doc().querySelector('[aria-label="Select all trashed projects"]'));doc().querySelector('[aria-label="Select all trashed projects"]').click();await sleep(25);w.confirm=()=>false;click('Delete permanently');await sleep(30);assert.equal(doc().querySelectorAll('.projectCard').length,1);w.confirm=()=>true;click('Delete permanently');await until(()=>doc().querySelectorAll('.projectCard').length===0);click('Your projects');await until(()=>doc().querySelectorAll('.projectCard').length===1);
 click('Help & About');await until(()=>doc().querySelector('dialog[open]'));click('About',doc().querySelector('dialog'));await sleep(30);assert(doc().querySelector('dialog').textContent.includes('Build 58'));click('Account',doc().querySelector('dialog'));await sleep(20);assert(doc().querySelector('dialog').textContent.includes('no account required'));click('Done',doc().querySelector('dialog'));
 dom.window.close();w=await boot();await until(()=>doc().querySelectorAll('.projectCard').length===1);assert(doc().querySelector('.projectHome'),'Startup must not jump into editor');
 doc().querySelector('.projectCover').click();await until(()=>!doc().querySelector('.projectHome'));assert(doc().querySelector('.saveBadge').textContent.includes('Episode renamed'));assert.deepEqual(errors,[]);
