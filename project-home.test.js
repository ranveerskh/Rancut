...
test('Home supports named projects, rename, duplicate, trash, restore and persisted reopen',async()=>{
 ...
 click('Help & About');await until(()=>doc().querySelector('dialog[open]'));click('About',doc().querySelector('dialog'));await sleep(30);assert(doc().querySelector('dialog').textContent.includes('Build '));click('Account',doc().querySelector('dialog'));await sleep(20);
