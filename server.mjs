...
export async function createApi({ffmpeg,exportDir,settingsPath}={}){
 ...
 const middleware=async(req,res,next=()=>json(res,404,{error:'Not found'}))=>{
  ...
  try{
   ...
   if(req.method==='GET'&&parts[1]==='health')return json(res,200,{ffmpeg:!!ffmpeg,version:'0.5.0',encoder:encoderInfo.selected,hardwareEncoders:encoderInfo.hardware,encoderMode:encoderInfo.mode,gpu:encoderInfo.gpu,ffmpegSource:encoderInfo.ffmpegSource});
   ...
   if(req.method==='GET'&&parts[3]==='download'){
    if(j.state!=='complete')throw Error('Export is not ready.');res.setHeader('Content-Type','video/mp4');res.setHeader('Content-Disposition','attachment; filename="RanCut-v0.5.0.mp4"');res.setHeader('Content-Length',(await stat(j.output)).size);await pipeline(createReadStream(j.output),res);return;
   }
   return next();
