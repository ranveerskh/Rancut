...
const finishClose=()=>{power.release();clearTimeout(closeTimer);allowClose=true;window?.destroy();};
if(!app.requestSingleInstanceLock()){app.quit();}else{
 ...
 app.whenReady().then(async()=>{try{
  ...
  const {newerVersion,releaseUrl}=await import(pathToFileURL(path.join(__dirname,'src/update-check.js')).href);
  ...
  ipcMain.handle('check-updates',async event=>{valid(event);const url=releaseUrl(releaseConfig.manifestUrl);
  ...
  ipcMain.on('close-failed',event=>{if(event.sender!==window?.webContents)return;clearTimeout(closeTimer);waiting=false;dialog.showMessageBox(window,{type:'error',buttons:['Keep editing','Close without local save'],defaultId:0,cancelId:0,noLink:true,message:'Your project could not be saved.',detail:'Keep editing to download a project backup with Save. Closing without a local save may lose your latest changes.'}).then(result=>{if(result.response===1)finishClose();}).catch(()=>{});});
