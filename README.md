# RanCut 0.4.1 — Clean creator update
...
- The workflow artifact points to the actual `RanCut-0.4.1-Setup.exe` filename.
...
`npm run dist:win` creates `release/RanCut-0.4.1-Setup.exe` and never publishes a GitHub release. The included GitHub Actions workflow runs the tests and uploads the installer and web build as separate artifacts. Do not rerun an old failed workflow attempt; run the workflow from the current `main` commit.
